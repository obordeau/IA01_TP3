...
(defun askQuestion (premisseConnu premisseInconnu)
    (if (and (null premisseConnu) (null premisseInconnu))
        (let* ((varInconnue 'COULEUR) (question (nth 0 *questions*)) (response NIL) (rPossible (responsePossible question)))
            ...
            (if (or (< response 0) (>= response (length rpossible))) (return-from askQuestion (askquestion premisseconnu premisseinconnu)))
                (push (list 'eq varinconnue (cadr (nth response rpossible))) *bdf*)
        )
        (let* ((varInconnue (cadr premisseInconnu)) (question (getquestion premisseconnu varinconnue)) (response NIL) (rPossible (responsepossible question)))
            (format t "~%~S~%> " (cadr question))
            (clear-input)
            (setq response (read))
            (clear-input)
            (cond 
                ((eq varinconnue 'DEGRE)
                    ...
                    (if (<= response 5) (push '(<= DEGRE 5) *bdf*) (push '(> DEGRE 5) *bdf*))
                    (if (<= response 8) (push '(<= DEGRE 8) *bdf*) (push '(> DEGRE 8) *bdf*))
                    (if (< response 9) (push '(< DEGRE 9) *bdf*) (push '(>= DEGRE 9) *bdf*))
                )
                ((eq varinconnue 'PRIX)
                    (if (or (< response 0.5) (> response 3)) (return-from askQuestion (askquestion premisseconnu premisseinconnu)))
                    (if (< response 1.8) (push '(= PRIX 1.6) *bdf*) (push '(= PRIX 1.8) *bdf*))
                    (if (< response 2) (push '(< PRIX 2) *bdf*) (push '(>= PRIX 2) *bdf*))
                )
                (T
                    (if (or (< response 0) (>= response (length rpossible))) (return-from askQuestion (askquestion premisseconnu premisseinconnu)))
                    (push (list 'eq varinconnue (cadr (nth response rpossible))) *bdf*)
...
(defparameter *rulesbase* '(
    ...
    (((eq STYLE triple)(eq TYPE speciale))(eq STYLE special_tripel))
    (((eq STYLE triple)(eq TYPE abbaye))(eq BEER "Val dieu"))
    (((eq STYLE special_tripel)(= PRIX 1.6))(eq BEER "Queue de charrue"))
    (((eq STYLE special_tripel)(= PRIX 1.8)(eq AROME epices))(eq BEER "Gouden Carolus"))
    (((eq STYLE special_tripel)(= PRIX 1.8)(eq AROME agrumes))(eq BEER "Tripel Karmeliet"))
...
(defun parcours ()
    (if (NULL *bdf*) (askQuestion NIL NIL))
        (loop while (NULL SOL)
            do
                (dolist (rule *rulesbase*)
                    (if (and (= (length (getPremisses rule)) 1) (member (car (getPremisses rule)) *bdf*) (not (member (getConclusion rule) *bdf*)))
                        (progn
                            (push (getConclusion rule) *bdf*)
                            (if (eq (nth 1 (getConclusion rule)) 'BEER) (setq SOL 1))
                        )
                    )
                )
                (dolist (rule *rulesbase*)
                    (if (= (length (getPremisses rule)) 2)
                        (if (and (member (caar rule) *bdf*) (not (member (cdar rule) *bdf*)))
                            (askQuestion (caar rule) (cdar rule))
                        )
                    )
                )
        )
    ...
    (loop while (= new_fact 1) do
        ...
        (dolist (rule *rulesbase*)
            ...
            (dolist (premisse (getPremisses rule))
                (if (not (member premisse *bdf*))
                    (setq applicable 0)
                )
            )
            (if (= applicable 1) (progn (push (getConclusion rule) *bdf*) (setq new_fact 1) (format T "new fait : ~S" (getConclusion rule))))
        )
    )
    (print "pas de nouveaux faits déduits")
    (dolist (rule *rulesbase*)
        (if (= (length (getPremisses rule)) 2)
            (if (and (member (caar rule) *bdf* :test #'equal) (not (member (cdar rule) *bdf* :test #'equal)))
                (progn (askQuestion (caar rule) (cadar rule)) (setq new_fact 1))
            )
        )
    )
    (parcours)
